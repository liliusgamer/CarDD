        ...
        {loading ? (
          ...
          <>
            ...
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <div className="font-semibold text-blue-700">{post.author_display_name || post.author_username}</div>
                <div className="text-xs text-gray-500">@{post.author_username} • {new Date(post.created_at).toLocaleString()}</div>
              </div>
              <button
              ...
              {/* Nút chỉnh sửa và xóa cho tác giả hoặc admin */}
              {isLoggedIn && (user?.username === post.author_username || user?.is_staff) && (
                ...
                {!commentLoading && comments.filter(c => !c.parent).map(c => (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div>
                        <b>{c.author_display_name || c.author_username}</b> <span style={{ color: '#888', fontSize: 12 }}>@{c.author_username} • {new Date(c.created_at).toLocaleString()}</span>
                        ...
                        {c.replies.map(r => (
                          ...
                          >
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                              ...
                              <div>
                                <b>{r.author_display_name || r.author_username}</b> <span style={{ color: '#888', fontSize: 12 }}>@{r.author_username} • {new Date(r.created_at).toLocaleString()}</span>
